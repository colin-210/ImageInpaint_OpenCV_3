...
if uploaded_file is not None:
    ...
    # canvas component
    canvas_result = st_canvas(fill_color='white',
    ...
    if option == 'Telea':
        ...
        if res1 is not None:
            ...
            st.sidebar.markdown(get_image_download_link(result1, 'telea.png', 'Download Output Image'),
                                unsafe_allow_html=True)
        if res2 is not None:
            result2 = Image.fromarray(res2)
            st.sidebar.markdown(get_image_download_link(result2, 'ns.png', 'Download Output Image'),
                                unsafe_allow_html=True)
    elif option == 'NS':
        st.subheader('Result of NS')
    ...
    if res is not None:
        ...
        st.sidebar.markdown(get_image_download_link(result, 'output.pmg', 'Download Output Image'), unsafe_allow_html=True)